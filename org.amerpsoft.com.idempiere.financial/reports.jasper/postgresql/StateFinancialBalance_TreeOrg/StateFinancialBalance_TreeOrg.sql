-- StateFInancialBalance_TreeOrg V1
-- OrgTree Version4
...
LEFT JOIN (	
	...
	SELECT
		...
		SELECT 
		  ...
		  fas2.ad_org_id,
		  $P{C_ElementValue_ID} as account_id,
		...
		FROM (
			SELECT
				fas22.ad_client_id,
				fas22.ad_org_id,
				fas22.account_id, fas22.C_AcctSchema_ID, 
				to_char(per2.startdate,'DD-MM-YYYY') AS dateini,
				to_char(per2.enddate,'DD-MM-YYYY') AS dateend,
				SUM(CASE WHEN (fas22.DateAcct < per2.StartDAte) THEN (fas22.amtacctdr - fas22.amtacctcr) ELSE 0 END) as openbalance,
						SUM(CASE WHEN (fas22.DateAcct >= per2.StartDAte AND fas22.DateAcct <= per2.EndDate) THEN COALESCE(fas22.amtacctdr,0) ELSE 0 END) as amtacctdr,
						SUM(CASE WHEN (fas22.DateAcct >= per2.StartDAte AND fas22.DateAcct <= per2.EndDate) THEN COALESCE(fas22.amtacctcr,0) ELSE 0 END) as amtacctcr,
						SUM(CASE WHEN (fas22.DateAcct <= per2.EndDate ) THEN (fas22.amtacctdr - fas22.amtacctcr) ELSE 0 END) as closebalance
				FROM (
					SELECT 
					fa22.ad_client_id, fa22.ad_org_id, fa22.fact_acct_id,
					fa22.c_acctschema_id, fa22.DateAcct, fa22.account_id, 
					fa22.amtacctdr, fa22.amtacctcr
					FROM fact_acct fa22
					LEFT JOIN c_elementvalue el2 ON (el2.c_elementvalue_id = fa22.account_id)
					LEFT JOIN (
						SELECT AD_Client_ID,C_Activity_ID, Value, Name FROM C_Activity WHERE isSummary='N' AND isActive='Y' AND AD_Client_ID=$P{AD_Client_ID}
					) as ac2 ON (ac2.AD_Client_ID = fa22.AD_Client_ID AND ac2.C_Activity_ID = fa22.C_Activity_ID)
					LEFT JOIN (
						SELECT AD_Client_ID,C_Project_ID, Value, Name FROM C_Project WHERE isSummary='N' AND isActive='Y' AND AD_Client_ID=$P{AD_Client_ID}
				...
				AND fas22.ad_client_id=$P{AD_Client_ID}
				GROUP BY fas22.ad_client_id, fas22.ad_org_id, fas22.account_id, 
						per2.startdate, per2.enddate, fas22.C_AcctSchema_ID
		) AS fas2
		LEFT JOIN C_ElementValue cel2 ON (cel2.C_ElementValue_ID = $P{C_ElementValue_ID})
		WHERE openbalance <> 0 
		   OR amtacctcr <> 0 
		   OR amtacctdr <> 0 
		   OR closebalance <> 0
		...
		WHERE ele.ad_client_id = $P{AD_Client_ID}  AND ele.issummary = 'N' AND ele.AccountType IN ('A','L','O')
