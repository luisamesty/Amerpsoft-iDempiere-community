...
1276000+c_city_id,0 AS ad_client_id,0 AS ad_org_id,'Y' AS isactive,now() AS created,0 AS createdby,now() AS updated,0 AS updatedby,city_name  as "name",
...
1276000+c_municipality_id AS c_municipality_id, 0 AS ad_client_id, 0 AS ad_org_id, 'Y' AS isactive, now() AS created, 0 AS createdby, now() AS updated, 0 AS updatedby, municipality_name AS "name", 
'' AS capital, 276 AS c_country_id, c_region_id,'N' AS isdefault, uuid_in(md5(random()::text || random()::text)::cstring) AS c_municipality_uu
FROM demografia 
...
(c_municipality_id, ad_client_id, ad_org_id, isactive, created, createdby, updated, updatedby, "name", capital, c_country_id, c_region_id, isdefault, c_municipality_uu)
	(SELECT DISTINCT ON (c_region_id, c_municipality_id) 
1276000+c_municipality_id AS c_municipality_id, 0 AS ad_client_id, 0 AS ad_org_id, 'Y' AS isactive, now() AS created, 0 AS createdby, now() AS updated, 0 AS updatedby, municipality_name AS "name", 
'' AS capital, 276 AS c_country_id, c_region_id,'N' AS isdefault, 
...
1276000+c_parish_id AS c_parish_id, 0 AS ad_client_id, 0 AS ad_org_id, 'Y' AS isactive, now() AS created, 0 AS createdby, now() AS updated, 0 AS updatedby, parish_name AS "name", 276 AS c_country_id, 
1027600+c_region_id AS c_region_id, 1276000+c_municipality_id AS c_municipality_id, 'N' AS isdefault, 
