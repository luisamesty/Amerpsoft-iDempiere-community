...
public class AddressValidationEventHandlerExt extends AbstractEventHandler implements EventHandler{
	...
	protected void doHandleEvent(Event event) {
		...
		if (topic.equals(IEventTopics.PO_BEFORE_NEW) || topic.equals(IEventTopics.PO_BEFORE_CHANGE)) 
		{
			...
			{
				MLocationExt location = (MLocationExt) po;
				...
				MAddressValidationExt validation = null;
				if (location.getC_AddressValidation_ID() > 0)
					validation = new MAddressValidationExt(location.getCtx(), location.getC_AddressValidation_ID(), location.get_TrxName());
				if (validation == null)
					validation = MAddressValidationExt.getDefaultAddressValidation(location.getCtx(), location.getAD_Client_ID(), location.get_TrxName());
				if (validation != null)
