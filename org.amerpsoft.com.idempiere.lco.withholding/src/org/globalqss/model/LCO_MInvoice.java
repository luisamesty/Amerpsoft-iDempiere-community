...
//log.warning("LCO_USE_WITHHOLDING="+MSysConfig.getBooleanValue("LCO_USE_WITHHOLDINGS", true, Env.getAD_Client_ID(Env.getCtx())) );
		...
		for (X_LCO_WithholdingType wt : wts)
		{
			...
			for (X_LCO_WithholdingRule wr : wrs)
			{
				...
				if (base != null &&
					...
					if (wc.getC_BaseTax_ID() != 0)
						iwh.setC_BaseTax_ID(wc.getC_BaseTax_ID());
