...
SELECT * FROM
	...
	LEFT JOIN adempiere.amn_s-- PayrollEmployeeLeavesAttendance
-- Payroll Employee Leaves and Attendance (BOTH)
-- 
-- ORDERED BY and Filtered  WorkForce
WITH date_series AS (
...
ORDER BY  leaves.emp_org_value, leaves.c_value, leaves.workforce_name, leaves.location_value, leaves.sector_name, leaves.emp_value, leaves.report_date, header_info ASChift_detail AS amnshd ON amnshd.amn_shift_id = emp2.amn_shift_id AND amnshd.dayofweek = pyr_asp.dayofweek
	LEFT JOIN adempiere.amn_shift AS attsh ON attsh.amn_shift_id = pyr_asp.amn_shift_id
	LEFT JOIN adempiere.amn_shift_detail AS attshd ON attshd.amn_shift_id = emp2.amn_shift_id AND attshd.dayofweek = pyr_asp.dayofweek
	LEFT JOIN (
		SELECT ad_client_id, STRING_AGG(lt_reference, ', ') AS leaves_all_txt
