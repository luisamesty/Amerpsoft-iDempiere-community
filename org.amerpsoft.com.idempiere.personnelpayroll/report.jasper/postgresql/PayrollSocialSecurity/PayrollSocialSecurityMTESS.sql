...
-- PayrollSocialSecurityMTESS_V2  Version 2
		 ...
		 INNER JOIN adempiere.ad_orginfo as orginfo ON (org.ad_org_id = orginfo.ad_org_id)
		  ...
		  CROSS JOIN (		  
			  SELECT COUNT(*) AS bdays
				...
				WHERE
				    EXTRACT(ISODOW FROM serie) NOT IN (7)
				    AND serie NOT IN (
				        SELECT date1
				        ...
				        WHERE
				            ...
				            AND IsActive = 'Y'
				            AND Date1 >= COALESCE((SELECT CAST(amndateini AS DATE) FROM amn_period WHERE amn_period_id = $P{AMN_Period_ID}), DATE_TRUNC('month', CURRENT_DATE)::DATE)
				            AND Date1 <= COALESCE((SELECT CAST(amndateend AS DATE) FROM amn_period WHERE amn_period_id = $P{AMN_Period_ID}), (DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '1 month' - INTERVAL '1 day')::DATE)
				            AND COALESCE(C_Country_ID, 0) IN (0, 276)
				 )
			) AS businessday	  
...
(
SELECT 
	ad_client_id AS client_id, ad_org_id AS org_id,
	org_value,
	org_name,
	value2,
	name2,
	...
	FROM (
		...
		GROUP BY ad_client_id, ad_org_id, org_value, org_name, rep_logo, value2,name2, calcorder2, amndateend, isshow, c_value, c_tipo,
