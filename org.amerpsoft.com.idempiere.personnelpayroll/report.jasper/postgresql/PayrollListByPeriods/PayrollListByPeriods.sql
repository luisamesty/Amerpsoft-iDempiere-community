-- PayrollListByPeriods
-- Payroll Historic Report by Periods
SELECT * FROM
(
	-- REPORT HEADER
...
(
	SELECT DISTINCT
	 ...
	 -- PROCESS
	 prc.amn_process_id, prc.value as proceso_codigo, COALESCE(prc.name, prc.description) as proceso_nombre,
     -- CONTRACT
     amc.amn_contract_id,
	 amc.value as contrato_value, COALESCE(amc.name, amc.description) as contrato_nombre, 
	-- CURRENCY
...
WHERE pyr.AD_Client_ID =$P{AD_Client_ID}
	...
	AND ( pyr.dateacct BETWEEN $P{DateIni} AND $P{DateEnd} )
	) as nomina ON (1= 0)
WHERE 
	(imp_header= 1) OR (client_id= $P{AD_Client_ID}
  AND ( CASE WHEN ( $P{AD_Org_ID}  IS NULL OR $P{AD_Org_ID} = 0 OR org_id= $P{AD_Org_ID} ) THEN 1=1 ELSE 1=0 END ) )
ORDER BY  nomina.proceso_codigo, nomina.contrato_value, nomina.amndateend, nomina.value_emp ASC, nomina.documentno ASC,   nomina.calcorder,nomina.isshow DESC , header_info ASC