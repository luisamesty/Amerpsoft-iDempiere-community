...
WHERE (imp_org= 1) OR (client_id= $P{AD_Client_ID}  AND org_id= $P{AD_Org_ID}
  AND imp_localidad= 1 AND imp_shift= 1 AND imp_contrato= 1 AND imp_empleado= 1
  AND fecha = DATE($P{DateEnd} )
  )
ORDER BY asistencia.loc_value, asistencia.codigo_shift, asistencia.value_emp, asistencia.fecha, asistencia.hora, org_inf ASC