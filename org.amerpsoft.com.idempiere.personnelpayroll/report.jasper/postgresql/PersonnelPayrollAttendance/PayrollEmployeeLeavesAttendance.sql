...
-- Version 3 Eliminados los retirados
WITH date_series AS (
...
SELECT * FROM
	...
	WHERE emp2.isActive = 'Y' AND emp2.AD_Client_ID= $P{AD_Client_ID} 
		...
		AND ( CASE WHEN ( $P{AMN_Employee_ID} IS NOT NULL AND $P{AMN_Employee_ID} != 0 ) THEN (emp2.amn_employee_id = $P{AMN_Employee_ID}) ELSE 1=1 END )
        AND ( CASE WHEN ( $P{AMN_Employee_ID} IS NULL OR $P{AMN_Employee_ID} = 0 ) THEN emp2.status != 'R' ELSE 1=1 END )
) as leaves ON (1= 0)
