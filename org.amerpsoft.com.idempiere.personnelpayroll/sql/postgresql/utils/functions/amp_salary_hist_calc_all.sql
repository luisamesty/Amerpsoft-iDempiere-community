-- Function: adempiere.amp_salary_hist_calc_all(character varying, numeric, timestamp without time zone, timestamp without time zone, numeric, numeric)
...
-- Calc is based on Dates Ini and End from AMN_Period Table Payroll Period Table from Receipts
--
--DROP FUNCTION IF EXISTS adempiere.amp_salary_hist_calc_all(character varying, numeric, timestamp without time zone, timestamp without time zone, numeric, numeric);
...
CREATE OR REPLACE FUNCTION adempiere.amp_salary_hist_calc_all(
...
END;	$BODY$
  ...
  COST 100;
ALTER FUNCTION adempiere.amp_salary_hist_calc_all(character varying, numeric, timestamp without time zone, timestamp without time zone, numeric, numeric)
  OWNER TO adempiere;
