-- DROP FUNCTION adempiere.amf_num2letterLPY(numeric, bpchar, bpchar);

CREATE OR REPLACE FUNCTION adempiere.amf_num2letterLPY(conv_number numeric, upp_low character, languaje_iso character)
...
BEGIN
	...
	-- CENTIMOS
	    ...
	    if entero > 0 then
                 case when languaje_iso = 'es' then en_letras := concat(en_letras , '') ;
                      when languaje_iso = 'en' then en_letras := concat(en_letras , '') ;
                      else en_letras := concat(en_letras , 'con 00/100 centimos') ;
                 end case;
	    else
                 case when languaje_iso = 'es' then en_letras := concat(en_letras , 'cero') ;
                      when languaje_iso = 'en' then en_letras := concat(en_letras , 'zero') ;
                      else en_letras := concat(en_letras , 'cero') ;
                 end case;	     
 	    end if;
        end if;     
	letter_number :=  en_letras;
