...
import org.adempiere.webui.apps.form.WFileImport;
...
public class AMNFileImport extends ADForm implements EventListener<Event>
{
	...
	private void dynInit()
	{
		...
		String TableName ="";
		MTable table = new MTable(Env.getCtx(), 0, null);
		
		String sql = MRole.getDefault().addAccessSQL("SELECT Name,AD_Impformat_ID, AD_Client_ID, AD_Table_ID FROM AD_ImpFormat WHERE isactive='Y'", "AD_ImpFormat",
		...
		try
		{
			...
			while (rs.next()) {
				table = new MTable(Env.getCtx(), rs.getInt(4), null);
				TableName = table.getTableName();
				// SELECT ONLY PERSONNEL IMPORT FORMATS 
				// Table Name STARTS WITH AMN_I
	...
	private void cmd_process()
	{
		...
		SessionManager.getAppDesktop().closeActiveWindow();
		//  
		AD_Table_ID= m_format.getAD_Table_ID();
		zoom(AD_Table_ID);
	}	//	cmd_process
	...
	public void zoom(int AD_Table_ID) {
		
		// Show Import Window
		int AD_WindowNo = getAD_Window_ID(AD_Table_ID,true);
		MQuery query = new MQuery(AD_Table_ID);
		AEnv.zoom (AD_WindowNo, query);
		//log.warning("Zoom: AD_WindowNo="+AD_WindowNo+"  AD_Table_ID="+AD_Table_ID);		
	}
	
	/**
	 *  getAD_Window_ID
	 * 	Get Zoom Window for import Table
	 *	@param int AD_Table_ID table 
	 *	@param boolean isSOTrx sales trx
	 *	@return AD_Window_ID
	 */
	protected int getAD_Window_ID (int AD_Table_ID, boolean isSOTrx)
	{
		...
		//
		String sql = "SELECT AD_Window_ID, PO_Window_ID FROM AD_Table WHERE AD_Table_ID=?";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
		...
		if (!isSOTrx && m_PO_Window_ID > 0)
			return m_PO_Window_ID;
		return m_SO_Window_ID;
	}	//	getAMN_I_Employee_Salary_AD_Window_ID
}
