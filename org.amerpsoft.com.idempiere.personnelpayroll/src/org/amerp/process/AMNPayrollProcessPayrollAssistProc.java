...
public class AMNPayrollProcessPayrollAssistProc {
			...
			Timestamp p_Shift_In2, Timestamp p_Shift_Out2)
	{
	    ...
	    if (amnshiftdetail == null) {
	    	sParamEmpty2=sParamEmpty2+Msg.getMsg(Env.getCtx(), "Error")+" "+
					...
					Msg.getElement(Env.getCtx(), "IsNonBusinessDay")+"\n";
	    	defattendancehours.setDescription(sParamEmpty2);
	    	defattendancehours.setIsNonBusinessDay(true);
		...
		if ( p_Shift_In1 == null || p_Shift_Out1 == null ) {
			...
			if (!bDescanso) {
				sParamEmpty1=sParamEmpty1+Msg.getMsg(Env.getCtx(), "Error")+" "+
					Msg.getElement(Env.getCtx(),"Shift_In1")+" / "+Msg.getElement(Env.getCtx(),"Shift_Out1")+" :"+
		...
		if ( p_Shift_In2 == null || p_Shift_Out2 == null) {
			...
			if (!bDescanso) {
				sParamEmpty2=sParamEmpty2+Msg.getMsg(Env.getCtx(), "Error")+" "+
						Msg.getElement(Env.getCtx(),"Shift_In2")+" / "+Msg.getElement(Env.getCtx(),"Shift_Out2")+" :"+
					Msg.getMsg(Env.getCtx(), "found.none")+"\n";
			} else { 
		...
		if ( defShift_In1 == null || defShift_Out1 == null || defShift_In2 == null || defShift_Out2 == null) {
			bParamEmpty=true;
			if (!bDescanso) {
				sParamEmpty=sParamEmpty+Msg.getMsg(Env.getCtx(), "Error")+"8 "+
				Msg.getElement(Env.getCtx(),"AMN_Shift_ID")+" "+
		...
		if (bParamEmpty) {
			attendancehours.setDescription(sParamEmpty);
		...
		int precision = 4;
		Shift_HED =  Shift_HED.divide(BD3600,precision, RoundingMode.CEILING);
		Shift_HEN =  Shift_HEN.divide(BD3600,precision, RoundingMode.CEILING);
		Shift_HND =  Shift_HND.divide(BD3600,precision, RoundingMode.CEILING);
		Shift_HNN =  Shift_HNN.divide(BD3600,precision, RoundingMode.CEILING);
		Shift_Attendance = Shift_Attendance.divide(BigDecimal.valueOf((long)2),precision, RoundingMode.CEILING);
		Shift_AttendanceBonus = Shift_AttendanceBonus.divide(BigDecimal.valueOf((long)2),precision, RoundingMode.CEILING);
		// *********************************************************
	...
	/**
	 * calcAttendanceValuesofPayrollVarsAdditional
			...
			MAMN_Shift_Detail amnshiftdetail, MAMN_Payroll_Assist_Proc attendancehours)
	{
		...
		// Verify if Any Shift Time is null 1st Half Time
		if ( attendancehours.getShift_In1() != null ) {
			// Shift_In1
			calShift_In1.setTime(attendancehours.getShift_In1());
			lShift_In1=secondsOfTime(calShift_In1, zeroCal);
		}
		if ( attendancehours.getShift_Out1() != null ) {
			// Shift_Out1
			calShift_Out1.setTime(attendancehours.getShift_Out1());
			lShift_Out1=secondsOfTime(calShift_Out1, zeroCal);
		}
		// Verify if Any Shift Time is null  2nd Half Time
		if (attendancehours.getShift_In2() != null ) {
			// Shift_In2
			calShift_In2.setTime(attendancehours.getShift_In2());
			lShift_In2=secondsOfTime(calShift_In2, zeroCal);
		
		}
		if (attendancehours.getShift_Out2() != null) {
			// Shift_Out2
			calShift_Out2.setTime(attendancehours.getShift_Out2());
			lShift_Out2=secondsOfTime(calShift_Out2, zeroCal);
		}	
		/// *********************
		// CALC EXTRA VARs
		/// *********************
		// Total Work Hours	
		if (lShift_In1 != 0 && lShift_Out1 !=0 && lShift_In2 != 0 && lShift_Out2 !=0) {
			if ((lShift_Out1 - lShift_In1 ) >= 0 ) {
				HR_HT = BigDecimal.valueOf((long) (lShift_Out1 - lShift_In1 ));
			}
			if ((lShift_Out2 - lShift_In2 ) >= 0 ) {
				HR_HT = HR_HT.add(BigDecimal.valueOf((long) (lShift_Out2 - lShift_In2)));
			}
			HR_HT = HR_HT.divide(BD3600,precision, RoundingMode.CEILING);
		}
		// Complete Hours 
		if (lShift_In1 != 0 && lShift_Out1 !=0 && lShift_In2 != 0 && lShift_Out2 !=0) {
			if ((lShift_Out2 - lShift_In1) >= 0 ) {
				HR_HC = BigDecimal.valueOf((long) (lShift_Out2 - lShift_In1));
				HR_HC = HR_HC.divide(BD3600,precision, RoundingMode.CEILING);
			}
		}
		// Free Hours 
		if (lShift_In1 != 0 && lShift_Out1 !=0 && lShift_In2 != 0 && lShift_Out2 !=0) {
			if ((lShift_In2 - lShift_Out1) >= 0 ) {
				HR_THL = BigDecimal.valueOf((long) (lShift_In2 - lShift_Out1));
				HR_THL = HR_THL.divide(BD3600,precision, RoundingMode.CEILING);
			}
		}
		// Free Hours Greater than 15 
		// Free Hours Less than 15
		if (HR_THL.compareTo(BigDecimal.ZERO) > 0 && lShift_In1 != 0 && lShift_Out1 !=0 && lShift_In2 != 0 && lShift_Out2 !=0) {
			// Comparar HR_THL con 15
			// Convertir horas a minutos (1 hora = 60 minutos)
	        BigDecimal HR_THL_InMinutes = HR_THL.multiply(new BigDecimal("60"));
	        // Comparar HR_THL_InMinutes con 15 minutos
	        if (HR_THL_InMinutes.compareTo(new BigDecimal("15")) > 0) {
	            // Si es mayor que 15 minutos, asignar a HR_HLGT15
	            HR_HLGT15 = HR_THL;
	            HR_HLLT15 = BigDecimal.ZERO;
	        } else {
	            // Si es menor o igual a 15 minutos, asignar a HR_HLLT15
	        	HR_HLGT15 = BigDecimal.ZERO;
	            HR_HLLT15 = HR_THL;
	        }
		}
		// Late Arrivals 
		if (lShift_In1 != 0 && ldefShift_In1 !=0) {
			if ((ldefShift_In1 - lShift_In1) >= 0 ) {
				HR_LTA = BigDecimal.valueOf((long) (ldefShift_In1 - lShift_In1));
				HR_LTA = HR_LTA.divide(BD3600,precision, RoundingMode.CEILING);
			}
		}
		// Early Departure 
		if (lShift_Out2 != 0 && ldefShift_Out2 !=0) {
			if ((ldefShift_Out2 - lShift_Out2) >= 0 ) {
				HR_EDE = BigDecimal.valueOf((long) (ldefShift_Out2 - lShift_Out2));
				HR_EDE = HR_EDE.divide(BD3600,precision, RoundingMode.CEILING);
			}
		}
		// Extra Holiday Hours 
			...
			Timestamp p_Event_Date, Integer p_AMN_Shift_ID, MAMN_Shift_Detail amnshiftdetail)
	{
		...
		if ( defShift_In1 == null || defShift_Out1 == null || defShift_In2 == null || defShift_Out2 == null) {
			bParamEmpty=true;
			if (!bDescanso) {
				sParamEmpty=sParamEmpty+Msg.getMsg(Env.getCtx(), "Error")+"11 "+
						Msg.getElement(Env.getCtx(), "AMN_Shift_ID")+" "+
						Msg.getMsg(Env.getCtx(), "default")+" "+Msg.getMsg(Env.getCtx(), "found.none")+"\n";
			} else { 
				sParamEmpty=sParamEmpty+Msg.getElement(Env.getCtx(),"Descanso"+" ");
