...
# and Aaron Axelsen <axelseaa@amadmax.com>
# Adapted to PostgreSQL 10 by Luis Amesty <luisamesty@gmail.com
#
...
# last Directory Added
if [ ! -e "$BACKUPDIR/last" ]	# Check Last Directory exists.
	then
...
# ---------------------------------------------------
dbdump () {
/opt/PostgreSQL/10/bin/pg_dump --username=$USERNAME $HOST $OPT $1 > $2
return 0
}

# -----------------------------------------------------
# Compression function
#   Compress sql file to selected compression mode
# -----------------------------------------------------
SUFFIX=""
compression () {
if [ "$COMP" = "gzip" ]; then
...
# -------------------------------------
# Database copylast function
#   Copy last backup to last directory
# -------------------------------------
copylast () {
cp $1 $2
return 0
...
if [ "$DBNAMES" = "all" ]; then
	DBNAMES="`/opt/PostgreSQL/10/bin/psql -U $USERNAME $HOST -l -A -F: | sed -ne "/:/ { /Name:Owner/d; /template0/d; s/:.*$//; p }"`"
...
echo ======================================================================
	...
	if [ $DOM = "01" ]; then
		...
		do
			...
			# last Directory Added
			if [ ! -e "$BACKUPDIR/last/$DB" ]		# Check Weekly DB Directory exists.
				then
				mkdir -p "$BACKUPDIR/last/$DB"
			fi
			echo Monthly Backup of $MDB...
				dbdump "$MDB" "$BACKUPDIR/monthly/$MDB/${MDB}_$DATE.$M.$MDB.sql"
				copylast "$BACKUPDIR/monthly/$MDB/${MDB}_$DATE.$M.$MDB.sql" "$BACKUPDIR/last/$MDB/${MDB}.sql"
	...
	if [ $DNOW = $DOWEEKLY ]; then
		...
		echo
			...
			copylast "$BACKUPDIR/weekly/$DB/${DB}_week.$W.$DATE.sql" "$BACKUPDIR/last/$DB/${DB}.sql"
	...
	# Daily Backup
		...
		echo
			...
			copylast "$BACKUPDIR/daily/$DB/${DB}_$DATE.$DOW.sql" "$BACKUPDIR/last/$DB/${DB}.sql"
...
echo ======================================================================
	...
	if [ $DOM = "01" ]; then
		echo Monthly full Backup of \( $MDBNAMES \)...
			...
			copylast "$BACKUPDIR/monthly/$DATE.$M.all-databases.sql" "$BACKUPDIR/last/all-databases.sql"
	...
	if [ $DNOW = $DOWEEKLY ]; then
		...
		echo
			...
			copylast "$BACKUPDIR/weekly/week.$W.$DATE.sql" "$BACKUPDIR/last/all-databases.sql"
	...
	# Daily Backup
		...
		echo
			...
			copylast "$BACKUPDIR/daily/$DATE.$DOW.sql" "$BACKUPDIR/last/all-databases.sql"
